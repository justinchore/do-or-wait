...
## ⏸️ Open items — pick up here first (updated 2026-08-07)

**Most recent thread (2026-08-07): Yardi sync project, step 1 of 6 done.** Justin's real problem: leads originate in Yardi Deal Manager but he manually re-enters them into Do-or-Wait, and some slip through. Full plan: (1) pull his assigned Yardi leads into Firestore every 20 min if missing, (2) keep follow-up-generation/reply-detection working, (3) push Do-or-Wait stage changes back to Yardi every 20 min. **Decided:** this sync runs as a **Claude cloud scheduled task** (`create_trigger`), NOT n8n — Justin's n8n is Managed Cloud with no server access, so Playwright/browser automation can't run there. Step 1 (done this session): replaced `leads/{id}.stage` taxonomy to match Yardi's vocabulary 1:1 (see Firestore collections below) and ran a one-time migration of all 97 leads. **Still open (steps 2-6, none built yet):**
1. Store Yardi login credential somewhere reachable over HTTPS (planned: `config/yardi_credential` Firestore doc — no proper secrets vault exists in this environment).
2. Build the Playwright **pull** script (log in as Justin, read assigned deals, intercept `dealmanager/api/deal/getlist`, create missing leads keyed on `yardi_deal_id`). Note: Yardi's own "My Deals" UI filter didn't reliably stick in manual testing — needs a robust Playwright-level fix, not plain clicks.
3. Build the **push** script (write Do-or-Wait stage changes back into Yardi by driving its UI — no direct-write API). Per Justin: **auto-write but log every change** (planned `yardi_sync_log` collection, not yet designed) — this deliberately reverses the project's standing "confirm before touching Yardi" caution, so the log is a hard requirement before going live.
4. Wire up the actual 20-minute scheduled task (both directions) + a login-failure health check.
5. End-to-end manual test of one full pull+push cycle before leaving it unattended.
6. `git push` the `index.html` taxonomy change promptly if not already done (see Deploy note under Leads below) — until pushed, the live site renders new stage values through old code and shows blank/fallback labels.
...
| 20 | Firestore Writer (generic) | **LIVE** — the generic `{collection,action,items|fields}` → Firestore batchWrite webhook, used by many other workflows (23, 25) and one-off migrations. Keep active. |
