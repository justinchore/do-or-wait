...
- **Follow-up same session: added ✅ Toured / 🔁 Reschedule / ✗ Cancelled quick actions**, prompted by realizing (while manually updating 4 tour leads via chat — see below) that these status changes should be a couple of taps, not a chat round-trip. `renderTourActionsBlock(lead)` renders a small "📅 Tour: <date> · Toured / Reschedule / Cancelled" row inside the expanded card (shown only when `stage==='tour'` and not archived), right above `renderLeadDetails`.
  - **✅ Toured** (`markTourDone`) logs a `met` entry and clears `tour_date`/`tour_time` (drops it off the strip). Deliberately does NOT advance stage/milestone — Yardi's tour-stage flow still has credit-app steps after touring before it becomes a Proposal, and guessing which one happened would be wrong more often than right.
  - **🔁 Reschedule** (`openRescheduleTour`) is a thin wrapper that opens the same Edit Details modal focused on the tour date/time fields (`openEditLeadDetails(id, e, {purpose:'reschedule'})`, tracked via the new `leadModalPurpose` global). `saveLeadDetails()` checks that flag and auto-logs the before/after as a thread note (e.g. "🔁 Tour rescheduled: Aug 18 3:30 PM → Aug 22 10:00 AM") so a reschedule is never a silent edit.
  - **✗ Cancelled** (`openTourCancel` → shared `#stage-resolve-overlay` modal, see below) always asks Dead-vs-back-to-Lead first (never guesses), and if Dead, asks which of the 13 Yardi Dead-Deal-Reason values. Logs a note either way and clears the tour date.
...
- **Also done this session, live in Firestore (no code involved):** backfilled `tour_date`/`tour_time` on the 4 tour-stage leads that already had a real scheduled date sitting in the old overloaded `next_due_date` field — RzImaging/Luis Rodriguez (8/18 3:30pm), Alex Hernandez (8/17 9am), Miguel Serrano (8/21, time still tentative — pending his confirmation), Jimmy Montalvo (8/20, also tentative). Also resolved the other 4 tour-stage leads Justin flagged as having no date: Mark Briseno moved to `dead`/"No Longer Interested" (toured 7/16, went cold after the 8/5 break-up email), Christian Kim logged as toured (Ontario Airport, stage left as `tour` pending next step), JNB Logistics and Rick Escobar left as-is (no confirmed date yet).
- **Same-session follow-up #2: 🏁 "Update stage" quick action on follow-up-due nags.** Justin's example: SSC Cargo Lines/Serami Baick kept showing as follow-up-due despite being a fully signed, paid, moved-in deal. Root cause: her lead doc had `stage: "won"` — a **leftover pre-2026-08-07 legacy stage value** (the taxonomy migration that day only touched the 97 leads that existed at the time; this one either predates that or got reset by something since). `isFollowupDue()`'s closed-stage check (`['executed','dead'].includes(...)`) only recognizes the *current* taxonomy, so `"won"` fell through as if still active. **Fixed live in Firestore:** `l1784139289135g` stage → `executed`, `yardi_milestone` cleared, note logged explaining the correction. **Same bug spotted in a second lead** (`l1780516645971`, SunStrong Management, `stage: "contract_sent"`) but left alone — deal's still genuinely open (agreement out for e-signature, not yet returned) and `followup_due` happens to read false right now anyway, so there was no clear right answer to pick unilaterally; flagging here in case it resurfaces.
  - **Generalized the tour-cancel overlay into a shared `#stage-resolve-overlay`** (was `#tour-cancel-overlay`/`tc-*` ids, renamed to `sr-*` — safe since none of this had been deployed yet) rather than building a second near-identical modal. `openStageResolve(id, e, opts)` takes `{altOutcome, title, questionPre, questionPost, altLabel}`; `outcome` is `'dead'` (always asks which of the 13 reasons) or `opts.altOutcome`. Two callers: `openTourCancel()` (`altOutcome:'lead'`, "Back to Lead") and the new `openFollowupResolve()` (`altOutcome:'executed'`, "Mark Executed (Won)"). `finalizeStageResolve(outcome, reason)` does the actual write + logs a note either way.
  - **🏁 Update stage button** added in three places that all funnel into `openFollowupResolve()`: the inline auto follow-up block (`renderFollowupBlock`), the Claude-generated follow-up block (`renderFollowupDraftBlock`), and the daily follow-up queue modal (`#fuqueue-overlay`, new `fuqResolveStage()` — hidden for space-match queue items via `#fuq-resolve-btn`'s display toggle, since "update stage" isn't meaningful there). `finalizeStageResolve()` also calls `renderFuqCard()` when the queue modal is open, so resolving a lead from inside the queue makes it self-heal/advance the same way skip/snooze/archive already do.
