...
**New-Outlook host fix (2026-06-25).** Graph started returning `webLink`s on the **new Outlook host** `outlook.cloud.microsoft/mail/deeplink/read/…`, whose deeplink reader fails with *"This message might have been moved or deleted"* — for **valid, current** messages, regardless of signed-in account, and the classic `outlook.office365.com/owa/?ItemID=…&viewmodel=ReadMessageItem` form fails too (it redirects into the same broken reader). This broke **every** stored link at once. Root cause is Microsoft-side (see office-js #1095). Fix: the **identical** deeplink on the `outlook.office365.com` host opens fine — *only the host differs*. So `fixThreadUrl(u)` in `index.html` rewrites `outlook.cloud.microsoft` → `outlook.office365.com` at **render time** (applied to both `renderEmailLinks` rows and `emailLinkChips` hrefs), fixing all already-stored links with no re-resolve. Workflow 11's "Pick newest hit" node applies the same rewrite before storing, so new links save in the working form. NOT a folder-move/stale-ID issue — re-resolving doesn't help; the host rewrite is the fix.
