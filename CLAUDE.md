...
**Workflow 33 was found sending qualifying emails without ever advancing the lead's milestone (2026-08-25).** A lead auto-qualified by workflow 33 stayed on whatever `yardi_milestone` the Yardi sync gave it (e.g. "Lead Assigned") indefinitely — the app's stage-bump-on-send logic only runs from manual in-app send buttons, never from an n8n webhook write, so automated sends bypassed it entirely. Fix built same day: `n8n/35_advance_lead_milestone.json` — a new generic milestone-advancer webhook (`advance-lead-milestone`) that maps a `{leadId, event}` POST to a forward-only `{stage, yardi_milestone}` write (never regresses a lead that's moved on; executed/dead stay human-only), plus `n8n/33_initial_qualifying_email.UPDATED.json` which adds an "Advance milestone (qualifying email sent)" call right after the qualifying send. **Status:** both pieces are now live (confirmed 2026-08-25) — workflow 33 published/imported in-place via its own ⋯ → Import from File, and workflow 35 imported as a new workflow and activated. **Still needs:** verify end-to-end on the next real qualifying-email send (lead should land on `yardi_milestone: "Lead Contacted (Sales)"` automatically with no manual fix) — hasn't been confirmed against a live Yardi-sourced lead yet since both went live. One lead (Walid Bakrou / Elan General Trading, La Mirada) was caught and manually corrected before this fix; a duplicate lead accidentally created for the same contact under "Ship 2 Leb" was merged/archived with `merged_into_lead_id`.
