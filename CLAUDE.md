...
## Floor plans = UPLOADED Site Plan images (CURRENT approach — decided 2026-06-17)
...
**Removed from `index.html`:** the `.xlsx` importer (`fpImportXlsx` + SheetJS reconstruction), the **⚙ Generate** button + `triggerFloorplanSync`/`FLOORPLAN_WEBHOOK`. **`n8n/8_floorplan_sync.json` is left in the repo but unused/deprecated** — do not import it. The two sections below are kept as a record of what was tried and why it didn't work.
